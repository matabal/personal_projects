...
public:
    ...
    void addVertex(Vertex<T> &);                            //Adds the passed vertex to the graph (with no edges).
    void addEdge(const int &, const int &, const double &); //Adds a weighted edge from the first vertex to the second.

    void removeVertex(const int &);            //Removes the given vertex. Also deletes any incident edges.
    ...
    size_t degree(const int &);    //Returns inDegree + outDegree
    ...
    Vertex<T> getVertex(const int &u_id);                   //Returns specified vertex. If vertex doesn't exist, the id and weight of the returned vertex are both -1.
...
{
    if (vertices.find(u_id) == vertices.end())
    {
        return false;
    } //This line uses std::unordered_map's find() function to search for the given ID in the vertices list
    else
    {
        return true;
    }
...
bool DirectedGraph<T>::adjacent(const int &u_id, const int &v_id)
{
    if (adj_list.find(u_id) != adj_list.end()) //First checks if the root vertex has any neighbours by checking if it exists in the adjacency list
    {
        ...
        {
            return true;
        } //Then checks if target vertex is a neighbour of the root
        return false;
    }
    return false;
    //Time Complexity: Average: O(1), Worst-Case: O(n)
...
void DirectedGraph<T>::addEdge(const int &u_id, const int &v_id, const double &weight)
{
    ...
    if (contains(u_id) && contains(v_id) && weight >= 0)
    {
        ...
        { //Checks if given vertices are already adjacent or not
...
size_t DirectedGraph<T>::inDegree(const int &u_id)
{
    ...
    for (auto &x : adj_list)
    {
        ...
        {
            s++;
        } //Increments the variable whenever it finds an edge pointing to the given vertex
    }
    return s;
    //Time Complexity: Average: O(n), Worst-Case: O(n^2)
}
...
std::vector<Vertex<T>> DirectedGraph<T>::getNeighbours(const int &u_id)
{
    ...
    if (!adj_list[u_id].empty()) //Checks if given vertex has any neighbours
    {
        ...
        {
            neighbours.push_back(vertices[x.first]);
        }
    ...
    while (!track.empty()) //Stack being empty indicates all reachable graph is traversed hence loop until it is empty, hence this while loop will run until this condition is met
    {
        ...
        while (it != adj_list[curr_id].end() && !found) //
        {
            if (!visited.at(it->first)) //Checks if current vertex is already visited
            {
                ...
                {
                    return true;
                } //This if-clause returns true if current vertex is vertex to be found hence indicating it is reachable from given former vertex
        ...
        {
            track.pop();
        } //If no unvisited neighbour is found, we are done with that vertex hence pop it out of stack.
    }
    return false;
...
std::pair<double, std::vector<Vertex<T>>> DirectedGraph<T>::shortestPath(int u_id, int v_id)
{
    ...
    while (!reached)
    {
        ...
        {
            reached = true;
        }                                                                  //break the loop after this iteration if root vertex is reached
                ...
                if (!found) //If no new unvisited vertex is found it means we have to start dft backtrack
                {
                    ...
                    {                                             //It is an extremely crucial line to this algorithm because a link value and a low link value being the same whilst backtrack indicates the tarjan stack currently stores one complete strongly connected component and the top element of the stack is the beginning of it (see Tarjan's algorithm)
                        ...
                        while (!reached)                          //until required amount of vertices are pooped
                        {
                            ...
                            if (tarjan_top == curr_id)
                            {
                                reached = true;
                            }                                                                //This line is to check if currently we are at the last element of the strongly connected component. If so mark reached to be true hence the loop will iterate once more and it will stop.
                            ...
                            {
                                reached = true;
                            } //This line is to protect the code from Segmentation Fault. If the tarjan stack is empty, the loop will break
                        }
...
std::vector<Vertex<T>> DirectedGraph<T>::topologicalSort()
{
    ...
    {
        containsCycles();
    } //If the graph is still not checked for cycles, check it.
